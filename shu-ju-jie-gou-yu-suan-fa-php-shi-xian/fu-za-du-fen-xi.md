...
算法的时间复杂度实际上就是算法的运行花费的时间 . 但是并不代表代码真正的执行时间 , 而是表示**代码执行时间随数据规模增长的变化趋势** , 所以也叫**渐进时间复杂度** , 简称**时间复杂度** .
...
**第一种方式 : 只关注循环执行次数最多的一段代码**

在分析一个算法 , 一段代码的时间复杂度的时候 , 只关注循环执行次数最多的那一段代码 . 这段核心代码执行次数的n的量级 , 就是整段要分析代码的时间复杂度 .
...
**第二种方式 : 加法法则 , 总复杂度等于量级最大的那段代码的复杂度**
